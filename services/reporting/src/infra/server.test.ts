...
function startWith(
  ...
  const server = createHttpServer({
    router: {
      handle: () =>
        Promise.resolve({ status: 200, body, ...(headers === undefined ? {} : { headers }) }),
    },
    probes: [],
    logger: silent,
    port: 0,
...
describe("returning a file rather than data", () => {
  it("writes the text as it is when the route asked for a file", async () => {
    running = startWith("day,delivered\n2026-09-07,40\n", {
      "content-type": "text/csv; charset=utf-8",
    });

    const response = await fetch(`${running.url}/v1/reports/x`);

    expect(await response.text()).toBe("day,delivered\n2026-09-07,40\n");
    expect(response.headers.get("content-type")).toContain("text/csv");
  });

  it("still sends data as json when nothing said otherwise", async () => {
    running = startWith({ ok: true });

    const response = await fetch(`${running.url}/v1/reports`);
