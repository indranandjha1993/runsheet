...
import { postgresPolicy } from "./adapters/repository.js";
import { describeConfig, policyConfig } from "./infra/config.js";
import { policyRoutes } from "./infra/routes.js";
import { createHttpServer, serviceLogger } from "./infra/server.js";

const config = policyConfig();
const logger = serviceLogger("policy", config.LOG_LEVEL);
const pool = new Pool({ connectionString: config.DATABASE_URL_POLICY });
...
const router = createRouter(
  policyRoutes({
    ...
    repository: postgresPolicy(pool),
    publisher: {
...
server.listen(config.PORT_POLICY, () => {
  logger.info("policy service listening", describeConfig(config));
});
